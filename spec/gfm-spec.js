describe("GitHub Flavored Markdown grammar", function () {
  ...
  it("tokenizes ***bold italic*** text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is ***bold\nitalic***!"),
    );
  ...
  it("tokenizes ___bold italic___ text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is ___bold\nitalic___!"),
    );
    expect(firstLineTokens[0]).toEqual({ value: "this is ", scopes: ["source.gfm"] });
    expect(firstLineTokens[1]).toEqual({
      value: "___",
  ...
  it("tokenizes **bold** text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is **not\nbold**!"),
    );
  ...
  it("tokenizes __bold__ text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is __not\nbold__!"),
    );
  ...
  it("tokenizes _italic_ text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is _not\nitalic_!"),
    );
  ...
  it("tokenizes ~~strike~~ text", function () {
    ...
    const [firstLineTokens, secondLineTokens] = Array.from(
      grammar.tokenizeLines("this is ~~str\nike~~!"),
    );
    expect(firstLineTokens[0]).toEqual({ value: "this is ", scopes: ["source.gfm"] });
    expect(firstLineTokens[1]).toEqual({
      value: "~~",
  ...
  it("tokenises code-blocks with borders of differing lengths", function () {
    let [firstLineTokens, secondLineTokens, thirdLineTokens] = Array.from(
    ...
    [firstLineTokens, secondLineTokens, thirdLineTokens] = Array.from(
      grammar.tokenizeLines("~~~~~~~\nfoo bar\n~~~"),
    );
    expect(firstLineTokens[0]).toEqual({
      value: "~~~~~~~",
  ...
  it("tokenizes mentions", function () {
    ...
    ({ tokens } = grammar.tokenizeLine(
      "sentence with a space before @name/ and an invalid symbol after",
    ));
    expect(tokens[0]).toEqual({
    ...
    ({ tokens } = grammar.tokenizeLine(
      "a username @1337_hubot with numbers, letters and underscores",
    ));
    ...
    ({ tokens } = grammar.tokenizeLine(
      "any email like you@domain.com shouldn't mistakenly be matched as a mention",
    ));
    expect(tokens[0]).toEqual({
  ...
  it("tokenizes HTML entities in *italic* text", function () {
    ...
    expect(tokens[1]).toEqual({
      ...
      scopes: [
        ...
        "punctuation.definition.entity.gfm",
      ],
    });
    expect(tokens[2]).toEqual({
      value: "trade",
  ...
  it("tokenizes YAML front matter", function () {
    const [firstLineTokens, secondLineTokens, thirdLineTokens] = Array.from(
      grammar.tokenizeLines(`\
---
front: matter
---\
...
`),
    ...
    expect(headingTokens[2]).toEqual({
      value: "Heading",
      scopes: ["source.gfm", "markup.heading.heading-1.gfm"],
    });
  });
  ...
  it("tokenizes criticmarkup", function () {
    const [addToken, delToken, hlToken, subToken] = Array.from(
      grammar.tokenizeLines(`\
Add{++ some text++}
Delete{-- some text--}
...
`),
    ...
    expect(addToken[1]).toEqual({
      ...
      scopes: [
        "source.gfm",
        "markup.inserted.critic.gfm.addition",
        "punctuation.definition.inserted.critic.gfm.addition.marker",
      ],
    });
    ...
    expect(delToken[1]).toEqual({
      ...
      scopes: [
        "source.gfm",
        "markup.deleted.critic.gfm.deletion",
        "punctuation.definition.deleted.critic.gfm.deletion.marker",
      ],
    });
    ...
    expect(subToken[1]).toEqual({
      ...
      scopes: [
        "source.gfm",
        "markup.changed.critic.gfm.substitution",
        "punctuation.definition.changed.critic.gfm.substitution.marker",
      ],
    });
    ...
    expect(subToken[3]).toEqual({
      ...
      scopes: [
        "source.gfm",
        "markup.changed.critic.gfm.substitution",
        "punctuation.definition.changed.critic.gfm.substitution.operator",
      ],
    });
